...
def test7():
    ...
    check50.run("./credit").stdin("1234567890").stdout("GECERSIZ\n").stdout(check50.EOF).exit(0)

@check50.check(compiles)
def test8():
    """identifies 369421438430814 as INVALID"""
    check50.run("./credit").stdin("369421438430814").stdout("GECERSIZ\n").stdout(check50.EOF).exit(0)

@check50.check(compiles)
def test9():
    """identifies 4062901840 as INVALID"""
    check50.run("./credit").stdin("4062901840").stdout("GECERSIZ\n").stdout(check50.EOF).exit(0)

@check50.check(compiles)
def test10():
    """identifies 5673598276138003 as INVALID"""
    check50.run("./credit").stdin("5673598276138003").stdout("GECERSIZ\n").stdout(check50.EOF).exit(0)
